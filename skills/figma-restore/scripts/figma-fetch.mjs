...
function loadFigmaToken(baseDir = CWD) {
  const envPath = join(baseDir, '.env.figma')
  let source
  try {
    source = readFileSync(envPath, 'utf8')
  } catch (error) {
    if (error?.code === 'ENOENT') return null
    throw new Error(`无法读取 .env.figma：${safeErrorMessage(error)}`)
  }
  for (const line of source.replace(/^\uFEFF/, '').split(/\r?\n/)) {
    const match = line.match(/^\s*(?:export\s+)?FIGMA_TOKEN\s*=\s*(.*?)\s*$/)
    if (!match) continue
    const value = match[1]
...
function iconTarget(icon, suffix = '') {
  const figmaName = String(icon.name || '').split('/').filter(Boolean).at(-1)
  const name = ICON_NAMES.get(figmaName)
    || (/^[A-Za-z0-9][A-Za-z0-9 ._-]*$/.test(figmaName) ? figmaName : `icon-${sanitizeNodeId(icon.id)}`)
  return `icons/${sanitizeName(name) || 'icon'}${suffix}.svg`
}

function iconTargets(icons) {
  const targets = new Map()
  const used = new Set()
  for (const icon of icons) {
    let target = iconTarget(icon)
    if (used.has(target)) target = iconTarget(icon, `__${sanitizeNodeId(icon.id)}`)
...
const VECTOR_TYPES = new Set(['VECTOR', 'BOOLEAN_OPERATION', 'ELLIPSE', 'LINE', 'POLYGON', 'STAR'])

function hasVisibleVector(node) {
  if (!node || node.visible === false) return false
  return VECTOR_TYPES.has(node.type) || (node.children || []).some(hasVisibleVector)
}

function hasVisibleTextOrImage(node) {
  if (!node || node.visible === false) return false
  if (node.type === 'TEXT' || (node.fills || []).some(fill => fill.visible !== false && fill.type === 'IMAGE')) return true
  return (node.children || []).some(hasVisibleTextOrImage)
}
...
function isIconInstance(node) {
  ...
  return node?.type === 'INSTANCE'
    ...
    && hasVisibleVector(node)
    && !hasVisibleTextOrImage(node)
}
...
function analyzeNode(node, bitmaps = [], icons = [], texts = []) {
  ...
  if ((exportSettings.some(item => item.format === 'SVG') || isIconInstance(node)) && !icons.some(item => item.id === node.id)) {
...
async function fetchNode(fileKey, nodeId, token, { baseDir = CWD, fetcher = fetchCompat } = {}) {
  ...
  const targets = iconTargets(icons)
  ...
  for (const icon of icons) {
    const target = targets.get(icon.id)
    try {
...
export { parseFigmaUrl, parseCliArgs, sanitizeName, sanitizeNodeId, bitmapTarget, iconTarget, iconTargets, loadFigmaToken, fetchCompat, apiFetch, download, analyzeNode, fetchNode }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    if (parsed.mode === 'help') printHelp()
      ...
      if (!token) throw new Error('缺少 .env.figma 中的 FIGMA_TOKEN')
